...
int send_login_message(char* username)
{
	...
	strncpy(login_msg.req_username,username, (USERNAME_MAX-1));
	data = &login_msg;
...
int send_join_message(char* channel)
{
	...
	strncpy(join_msg.req_channel,channel, CHANNEL_MAX-1);
	data = &join_msg;
...
int send_leave_message(char* channel)
{
	...
	strncpy(msg.req_channel,channel, CHANNEL_MAX);
	data = &msg;
...
int send_say_message(char* text)
{
	...
	strncpy(say_msg.req_channel,active_channel, CHANNEL_MAX);
	strncpy(say_msg.req_text,text, SAY_MAX);
	data = &say_msg;
				...
				if (parameter != NULL)
				{
					if (strlen(parameter) > 1)
					{
						...
						if (strncmp(remainder, "join ", 5) == 0)
						{
							...
							{
							strncpy(active_channel,parameter, CHANNEL_MAX);
							}
						...
						else if (strncmp(remainder, "who ", 4) == 0)
						{
							...
							if (channel_iter == subscribed_channels.end())
							{
								...
								strncpy(active_channel, parameter, (CHANNEL_MAX-1));
							}
						}else{
